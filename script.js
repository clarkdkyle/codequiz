...
function callquest() {
    questcount++;
    correctanswer = questions[questcount].answer;

    quests.textContent = questions[questcount].title;
    options.innerHTML = "";

    var answersopts = questions[questcount].answersopts;

    for (i = 0; i < answersopts.lenth; i++) {
        var newchoices = 
        document.createElement("button");
        newchoices.textContent = answersopts[i]
        crctbtn = options.appendChild(newchoices).setAttribute("class", "p-3 m-1 btn btn-light btn-block")
    }
}
...
// make it respond to incorrect timers

// get initials for high scores
...
function disscore() {
    ...
    $("#submit-score").removeClass('d-none');
    $('#user-score').text = "Final score is " + secondsLeft + ".";
}
// Instructions
